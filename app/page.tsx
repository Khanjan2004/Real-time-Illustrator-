...
export default function HomePage() {
  ...
  const prompt = useMemo(() => {
    if (mode === 'code') return CODE_PROMPTS[(typed.length + 3) % CODE_PROMPTS.length]
    if (mode === 'custom') return 'Paste your own custom prompt from settings (MVP uses this placeholder custom sentence).'
    if (mode === 'precision') return 'Type slowly and chase perfect accuracy over speed.'
    if (mode === 'meditation') return 'Breathe, type softly, keep a stable rhythm and reduce error spikes.'
    return LANG_PROMPTS[language]
  }, [mode, language, typed.length])

  const elapsedSeconds = startedAt ? Math.max(1, Math.floor((now - startedAt) / 1000)) : 0
  const typedChars = typed.length
  const correctChars = prompt.split('').filter((c, i) => typed[i] === c).length
  ...
  const consistency = Math.max(0, 100 - stdDev(timeline.map((t) => t.wpm)) * 2)
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTimeline((prev) => [...prev, { t: Math.max(1, Math.floor((Date.now() - startedAt) / 1000)), wpm: netWpm, acc: accuracy }])
    }, 1000)
  ...
  useEffect(() => {
    if (!startedAt) return
    if (mode === 'time' && elapsedSeconds >= duration) finishSession()
    if (mode === 'words' && typed.trim().split(/\s+/).filter(Boolean).length >= wordCount) finishSession()
  })
  ...
  return (
    <div className="space-y-4">
      <motion.h1 initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="text-3xl font-bold">Typing Test Arena</motion.h1>

      <div className="grid md:grid-cols-3 gap-3">
        <ModeSelector mode={mode} onChange={(m) => { setMode(m); setTyped(''); setTimeline([]); setStartedAt(null) }} />
        <div className="panel flex gap-2 items-center">
          <label className="text-sm">Language</label>
          <select className="bg-black/30 rounded p-2" value={language} onChange={(e) => setLanguage(e.target.value)}>
            <option value="en">English</option><option value="uz">O'zbek</option><option value="ru">Русский</option><option value="ar">العربية</option>
          </select>
        </div>
        <div className="panel flex gap-2 items-center">
          <label className="text-sm">Timer</label>
          <select className="bg-black/30 rounded p-2" value={duration} onChange={(e) => setDuration(Number(e.target.value))}>
            <option value={15}>15s</option><option value={30}>30s</option><option value={60}>60s</option><option value={120}>120s</option>
          </select>
          <label className="text-sm ml-2">Words</label>
          <select className="bg-black/30 rounded p-2" value={wordCount} onChange={(e) => setWordCount(Number(e.target.value))}>
      ...
      <div className="space-y-3" onClick={() => inputRef.current?.focus()}>
        ...
        <div className="panel flex items-center justify-between text-sm">
          ...
          <div className="space-x-2">
            <button onClick={() => { setTyped(''); setTimeline([]); setStartedAt(null); setBackspaces(0) }} className="px-3 py-1 border rounded-lg border-white/20">Reset</button>
            <button onClick={finishSession} className="px-3 py-1 rounded-lg bg-accent text-black">Finish</button>
          </div>
        </div>
      </div>
